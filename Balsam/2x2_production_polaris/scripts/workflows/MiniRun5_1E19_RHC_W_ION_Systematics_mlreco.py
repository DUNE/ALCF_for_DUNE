...
def submit_all_jobs(num_nodes=1, wall_time_min = 60):
    ...
    for start in range(begin_index, end_index, 128):
        BatchJob.objects.create(
            ...
            project=project,
            site_id=site.id,
            filter_tags={"workflow": f"{name}_analysis_{version}"},
...
if "flow2supera" in runs or "all" in runs:
    ...
    for start in range(begin_index, end_index, spill_size):
        for i in range(start, start+spill_size):
            if i < end_index:
                create_single_dependent_job("flow2supera", i, parent_job_ids, gpu_packing_count, start)
    ...
    for start in range(begin_index, end_index, 128):
        for i in range(start, start+spill_size):
            if i < end_index:
                ...
                # )]
                parent_job_ids = []
                create_single_dependent_job("inference", i, parent_job_ids, cpu_packing_count, start)
